...
impl GithubRepo {
    ...
    pub async fn gql_prs_for_branches(&self, branches: &HashSet<String>) -> anyhow::Result<Vec<Pr>> {
        ...
        ", self.current_user, self.org, self.repo);
        let pulls: serde_json::Value = self.octo.graphql(query.as_str())
            .await
            .map_err(anyhow::Error::msg)?;

        // HOLY CRAP!  GENERATED BY COPILOT (works pretty well though tbh).
        // Honestly, this should be a macro.
        let prs: Vec<Pr> = pulls.as_object()
            .unwrap()
            .get("data")
            .unwrap()
            .as_object()
            ...
            .iter()
            .map(|x| {
                let node = x.as_object().unwrap().get("node").unwrap().as_object().unwrap();
                Pr {
                    closed: node.get("closed").unwrap().as_bool().unwrap(),
                    title: node.get("title").unwrap().as_str().unwrap().to_string(),
                    branch: node.get("headRefName").unwrap().as_str().unwrap().to_string(),
